...
主要特性:
- 支持多种格式: EPUB, MOBI, AZW3, PDF, DOCX。
- 高性能: 利用多进程并行处理，大幅缩短转换时间。
- 健壮性: 对加密、内容为空、格式不规范等异常情况有良好的处理机制。
- 跨平台: 修正了平台相关的代码，可在主流操作系统运行。
- 灵活性: 支持通过命令行参数自定义进程数、超时时间、文件过滤等。
- 用户友好: 提供实时进度条、彩色状态输出和详细的日志报告。

使用示例:
  python run.py ./input ./output --threads 8 --clean-output
"""
...
try:
    # 检查 Calibre 的命令行工具 'ebook-convert' 是否在系统路径中
        ...
        for idref in spine_idrefs:
            ...
            if content_path_str in zf.namelist():
                try:
                    ...
                    logging.debug(f"处理EPUB内文件'{content_path_str}'失败: {e}")
            else:
                warnings_list.append(f"引用的章节文件'{content_path_str}'未在压缩包中找到")
        ...
        try:
            ...
            if temp_txt_path.exists():
                ...
                if not text_content.strip():
                    raise RuntimeError("Calibre 转换成功，但输出的文本文件为空。")
                return text_content, []
            else:
                raise RuntimeError("Calibre 转换过程未生成输出文件。")

        except FileNotFoundError:
             raise RuntimeError("无法执行 'ebook-convert'。请确保 Calibre 已安装并将其添加至系统环境变量PATH。")
...
"""
    ...
    parser.add_argument("-t", "--threads", type=int, default=os.cpu_count(),
                        help=f"处理进程数 (默认: 系统CPU核心数, {os.cpu_count()})")
...
def print_summary(stats: Dict[str, Any], duration: float, log_file: Path, c: Color):
    ...
    if stats['warnings']:
        print(f"{c.WARNING}产生警告: {stats['warnings']:>5} 文件 (详情请查看日志){c.ENDC}")
    if stats['errors']:
        print(f"{c.FAIL}发生错误: {stats['errors']:>5} 文件 (详情请查看日志){c.ENDC}")
    if stats['timeouts']:
        print(f"{c.FAIL}{c.BOLD}处理超时: {stats['timeouts']:>5} 文件{c.ENDC}")
...
def main():
    ...
    for ext, handler_installed in {'.docx': HAVE_DOCX, '.pdf': HAVE_PDF, '.mobi': HAVE_CALIBRE, '.azw3': HAVE_CALIBRE}.items():
        if not handler_installed:
            if ext in ['.mobi', '.azw3']:
                print(f"{c.WARNING}提示: 未找到 Calibre 环境，将无法转换 {ext} 文件。{c.ENDC}")
            else:
        ...
        if output_dir.exists():
            if args.clean_output:
                ...
                print(f"{c.FAIL}错误: 输出目录 '{output_dir}' 已存在。为防止数据丢失，请先手动删除该目录，\n"
                      f"      或使用 --clean-output 标志授权脚本进行清空。{c.ENDC}")
                sys.exit(1)
        ...
        for future in concurrent.futures.as_completed(future_map):
            ...
            try:
                ...
                if status == 'converted':
                    ...
                    if not messages:
                        tqdm.write(f"{c.OKGREEN}[已转换] {rel_path}{c.ENDC}")
                    else:
                        stats['warnings'] += 1
