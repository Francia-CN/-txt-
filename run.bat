@echo off
:: 使用 setlocal 和 endlocal 是一个好习惯，可以防止脚本中的变量污染用户的系统环境。
:: enabledelayedexpansion 允许在循环或IF语句中正确读取变化的变量（如此脚本中的!DEBUG_MODE!）。
...
::
::  功能特性:
::  - 自动检测并兼容 UTF-8 编码，防止中文乱码。
::  - 提供调试模式：启动时按住 Shift 键，脚本会分步执行，便于排查问题。
::  - 自动查找 Python 环境，无需手动配置。
::  - 支持双击运行（处理input目录）和拖拽文件夹两种模式。
::  - 对清空输出目录等危险操作进行交互式二次确认。
::  - 捕获 Python 脚本的执行错误，确保窗口不会闪退。
::
:: ============================================================================


:: --- 0. 调试模式检测 ---
:: 如果在双击此脚本时按下了SHIFT键，则激活调试模式，分步暂停执行。
if defined SHIFT (
    set "DEBUG_MODE=ON"
...
:: 切换代码页为UTF-8 (65001) 来正确显示中文字符，>nul 用于隐藏切换时的提示信息。
chcp 65001 >nul
...
:: 检查Python可执行文件 (优先查找python, 其次是python3)。
...
:: 切换到Python脚本所在的目录，然后用找到的Python解释器执行主脚本。
:: 将所有路径和参数用双引号括起来，可以完美处理带空格的路径。
cd /d "%~dp0"
!PYTHON_EXE! "!PYTHON_SCRIPT!" "!INPUT_DIR!" "!OUTPUT_DIR!" !CLEAN_FLAG!

:: 检查Python脚本的退出代码。如果非0，则表示执行过程中发生了错误。
if errorlevel 1 (
	echo.
